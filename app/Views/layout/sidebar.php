<!--**********************************
            Sidebar start
        ***********************************-->
<div class="deznav">
    <div class="deznav-scroll">
        <ul class="metismenu" id="menu">
            <!-- Ubah disini -->
            <li><a class="ai-icon" href="/home" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Dashboard</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/beasiswa" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Jenis Beasiswa</span>
                </a>
            </li>

            <li><a class="ai-icon" href="/penerima" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Data Penerima Beasiswa</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/akademik" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Laporan Akademik</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/prestasi" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Laporan Prestasi</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/mbkm" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Laporan MBKM</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/keaktifan" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Keaktifan per Semester</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/gform" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Daftar Link Gform</span>
                </a>
            </li>
            <li><a class="ai-icon" href="import.html" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Import</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/pengumuman" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Pengumuman</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/panduan" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Buku Panduan</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/manajemen" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Manajemen Pengguna</span>
                </a>
            </li>
            <li><a class="ai-icon" href="/log" aria-expanded="false">
                    <!-- <i class="la la-calendar"></i> -->
                    <span class="nav-text">Log Aktivitas Pengguna</span>
                </a>
            </li>

        </ul>
    </div>
</div>
<!--**********************************
            Sidebar end
        ***********************************-->